...
const Header = () => {
  ...
  return (
    <>
      <header
        class={`backdrop-blur-sm py-4 px-8 fixed top-0 inset-x-0 transition-all duration-500 z-20 ${
          isScrolled || navOpen
            ? 'bg-stone-50 text-stone-900 shadow'
          ...
          <nav class="flex-shrink hidden gap-4 md:flex">
            {NAVIGATION.map((item, i) => (
              <Link
                key={i}
                href={item.target}
                class="block transition-all hover:tracking-widest min-w-[8ch] text-center drop-shadow py-2 px-4"
      ...
      <div
        class={`fixed inset-0 w-full h-screen z-10 bg-stone-900 transition-opacity pointer-events-none ${
